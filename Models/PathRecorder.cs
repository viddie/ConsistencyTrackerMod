...
namespace Celeste.Mod.ConsistencyTracker.Models {
    public class PathRecorder {
        ...
        public string GetRecorderStatus() {
            ...
            string toRet = $"{Dialog.Clean("MODOPTION_CCT_RECORD_PATH_SETTINGS_PATH_MANAGEMENT_RECORDING_STATUS_TITLE")}:\n";
            int checkpointIndex = 0;
            foreach (List<string> checkpoint in Checkpoints) {
                string cpName = CheckpointNames[checkpointIndex];
                string cpAbbreviation = CheckpointAbbreviations[checkpointIndex];
                ...
                if (checkpointIndex == 0 && Checkpoints.Count == 1) {
                    cpName = Dialog.Clean("MODOPTION_CCT_RECORD_PATH_SETTINGS_PATH_MANAGEMENT_RECORDING_STATUS_ROOM");
                    cpAbbreviation = "R";
                }

                toRet += $"{cpName} ({cpAbbreviation}): {checkpoint.Count} {Dialog.Clean("MODOPTION_CCT_RECORD_PATH_SETTINGS_PATH_MANAGEMENT_RECORDING_STATUS_ROOMS")}\n";
