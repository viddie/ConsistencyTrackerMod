...
namespace Celeste.Mod.ConsistencyTracker {
    public class ConsistencyTrackerModule : EverestModule {
        ...
        private bool LogPosition, LogSpeed, LogVelocity, LogLiftBoost, LogFlags, LogInputs;
        private bool LogIsFrozen = false;
        ...
        private void Engine_Update(On.Monocle.Engine.orig_Update orig, Monocle.Engine self, GameTime gameTime) {
            LogIsFrozen = Engine.FreezeTimer > 0;
        ...
        private void LogPhysicsUpdate(Player player) {
            ...
            if (logPhysics && !LastLogPhysics) {
                ...
                PhysicsLogWriter.WriteLine(getPhysicsLogHeader(LogPosition, LogSpeed, LogVelocity, LogLiftBoost, LogFlags, LogInputs));
                PhysicsLogFrame = 0;
        ...
        private Dictionary<int, string> StatesToCheck = new Dictionary<int, string>() {
        ...
        public string getPlayerFlagsFormatted(Player player) {
            ...
            if (StatesToCheck.ContainsKey(player.StateMachine.State)) {
                flags += $"{StatesToCheck[player.StateMachine.State]} ";
            } else {
                flags += $"StOther ";
            }
